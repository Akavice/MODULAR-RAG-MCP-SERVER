...
| C11 | BM25Indexer?????+IDF??? | [x] | 2026-05-26 | Implemented `BM25Indexer` (inverted index + IDF rebuild + JSON persistence + remove_document/query); review passed with roundtrip + extra contract tests (9 passed). |
...
| H2 | CompositeEvaluator 实现 | [x] | 2026-06-02 | Implemented `observability.evaluation.CompositeEvaluator` to run multiple `BaseEvaluator` backends concurrently and merge numeric metrics with duplicate-name disambiguation. Fixed review-found overwrite bug by generating unique suffixed metric keys when three or more evaluators return the same metric name. Extended `EvaluatorFactory.create()` to support `evaluation.backends: [...]` and build a composite evaluator from string or mapping backend entries while preserving single-provider behavior. Added `tests/unit/test_composite_evaluator.py` covering metric merge, duplicate keys, invalid evaluator/result validation, factory-driven composite creation, and triple-duplicate metric preservation. Validation: H2 + evaluator suite `22 passed`; evaluation/config regression subset `28 passed`. |

| H3 | EvalRunner + Golden Test Set | [x] | 2026-06-02 | Implemented `observability.evaluation.EvalRunner` with golden-set JSON loading, case validation, retrieval execution, per-case hit_rate/mrr, evaluator metric aggregation, source fallback matching, and serializable `EvalReport`. Fixed review-found metric overwrite bug by preserving runner-owned source-aware `hit_rate/mrr` and prefixing evaluator metrics with conflicting names (for example `evaluator.hit_rate`). Added `tests/fixtures/golden_test_set.json` and implemented `scripts/evaluate.py` with default local dry-run output plus `--live` mode for real `HybridSearch` from settings. Added `tests/unit/test_eval_runner.py` for runner aggregation, source fallback with `CustomEvaluator`, invalid golden-set handling, top_k validation, and CLI JSON output. Validation: H3 suite `5 passed`; H1-H3 evaluator regression subset `27 passed`; `python scripts/evaluate.py` outputs metrics JSON. |
