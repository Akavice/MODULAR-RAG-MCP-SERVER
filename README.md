...
> Development status: Phase A1 scaffold initialized on 2026-04-20. Run `python main.py` for a minimal smoke check.
...
为提升可用性，Dashboard 将在 G7 阶段新增双语支持与右上角语言切换入口。

- 支持语言：`zh-CN`、`en-US`
- 切换位置：页面右上角（全局语言切换）
- 生效范围：G1-G6 六个页面（系统总览、数据浏览、Ingestion 管理、Ingestion 追踪、Query 追踪、评估面板）
- 会话记忆：语言状态保存在当前会话（`st.session_state`）
- 回退策略：当前语言缺失文案时回退到英文，再回退到 key 本身

说明：该能力当前处于规划中，具体开发任务见 `DEV_SPEC.md` 的 `G7`。
